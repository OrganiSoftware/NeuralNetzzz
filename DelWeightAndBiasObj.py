...
class DelWeightAndBiasObj:
    ...
    def average_del_weights_bias(self):
        dual_weight_calcs = DualNumber(self.num_weight_calcs, 0)
        if not self.averaged and not self.num_weight_calcs == 0:
            for weight_index in range(len(self.del_weights)):
                self.del_weights[weight_index] = self.del_weights[weight_index] / dual_weight_calcs
            self.del_bias = self.del_bias / dual_weight_calcs
            self.averaged = True
    ...
    def clear(self):
        self.del_bias = DualNumber(0, 0)
        for weight_index in range(len(self.del_weights)):
            self.del_weights[weight_index] = DualNumber(0, 0)
        self.averaged = False
        self.num_weight_calcs = 0
