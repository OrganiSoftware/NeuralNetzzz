            ...
            for perceptron_index in range(len(self.neural_net.neural_net[index].neural_layer)):
                ...
                if layer_index == 0:
                    ...
                    for temp_cost_index in range(len(temp_del_costs)):
                        temp_del_costs_matrix[temp_cost_index] += temp_del_costs[temp_cost_index]
                else:
                    for weight_index in range(len(self.neural_net.neural_net[index].neural_layer[perceptron_index].weights)):
                        if weight_index == 0:
                            del_bias = self.neural_net.neural_net[index].neural_layer[
                                                                            perceptron_index].comp_partial_for_mse_cost(None, True, del_costs_matrix[perceptron_index])
                        temp_del_weight = self.neural_net.neural_net[index].neural_layer[
                                               perceptron_index].comp_partial_for_mse_cost(weight_index, False, del_costs_matrix[perceptron_index])
                        temp_del_costs_matrix[weight_index] += self.neural_net.neural_net[index].neural_layer[
                                              perceptron_index].calc_del_c_not_del_activation(weight_index, del_costs_matrix[perceptron_index])
